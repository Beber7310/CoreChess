...
void initBook() {
	...
	book_loadInternal();
	readBookFile();
	//FIXME board_loadFromFen(STARTFEN);
}
...
smove bookCheckMove(sboard* pBoard, char* str) {
...
void book_addline(int line_no, char* movestring) {
	...
	for (int i = 0; i < limit / 5 + 1; i++) {
		...
		*********************************************/
		...
		//if (bookCheckMove(&board, local_string)) {
			book_add(&board, local_string);
		/*}
		else
		{
...
int book_add(sboard* pBoard, char* movestring) {
	...
	**************************************************************/
	...
	/*
	fromX = movestring[0] - 'a';
	fromY = movestring[1] - '1';
	toX = movestring[2] - 'a';
	toY = movestring[3] - '1';
	from = fromX + 8 * fromY;
	to = toX + 8 * toY;
	moveBuild(&curr_move, from, to, 0);
	*/

	curr_move = bookCheckMove(pBoard, movestring);

	if (book_present(pBoard,&curr_move)) {
		doMove(pBoard, &curr_move);
		return 1;
	}
	 
	book[book_index].hash = pBoard->_zobKey;
...
int book_present(sboard* pBoard, smove* curr_move) {
	...
	for (int i = 0; i < book_index; i++) {
		...
		if (book[i].hash == pBoard->_zobKey &&
			book[i].move._move == curr_move->_move) {
			book[i].freq++;
			return 1;
		}
	}
	...
	for (int i = 0; i < BOOK_SIZE; i++) {
		if (book[i].hash == pBoard->_zobKey) {
			...
			****************************************************/
			...
			if ((temp > priority) &&(book[i].freq != EXCLUDED)) {
				priority = temp;
...
void book_loadInternal() {
	...
	book_addline(0, "e2e4 b8c6 f1b5 g8f6");
	book_addline(0, "e2e4 d7d5 e4d5 d8d5 b1c3");
